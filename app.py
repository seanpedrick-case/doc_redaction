...
if DISPLAY_FILE_NAMES_IN_LOGS == "True": DISPLAY_FILE_NAMES_IN_LOGS = True 
else: DISPLAY_FILE_NAMES_IN_LOGS = False
if DO_INITIAL_TABULAR_DATA_CLEAN == "True": DO_INITIAL_TABULAR_DATA_CLEAN = True 
else: DO_INITIAL_TABULAR_DATA_CLEAN = False
if COMPRESS_REDACTED_PDF == "True": COMPRESS_REDACTED_PDF = True 
else: COMPRESS_REDACTED_PDF = False
if RETURN_PDF_END_OF_REDACTION == "True": RETURN_PDF_END_OF_REDACTION = True 
else: RETURN_PDF_END_OF_REDACTION = False
if USE_GREEDY_DUPLICATE_DETECTION == "True": USE_GREEDY_DUPLICATE_DETECTION = True 
...
if DEFAULT_TEXT_COLUMNS: DEFAULT_TEXT_COLUMNS = _get_env_list(DEFAULT_TEXT_COLUMNS)
if DEFAULT_EXCEL_SHEETS: DEFAULT_EXCEL_SHEETS = _get_env_list(DEFAULT_EXCEL_SHEETS)
...
with app:
    ...
    duplication_file_path_outputs_list_state = gr.Dropdown(label = "duplication_file_path_outputs_list", value=list(), multiselect=True, allow_custom_value=True,visible=False)
    ...
    recogniser_entity_dataframe_same_text = gr.Dataframe(pd.DataFrame(data={"page":list(), "label":list(), "text":list(), "id":list()}), col_count=(4,"fixed"), type="pandas", label="Table rows with same text", headers=["page", "label", "text", "id"], show_fullscreen_button=True, wrap=True, max_height=400, static_columns=[0,1,2,3], visible=False)

    to_redact_dataframe_same_text = gr.Dataframe(pd.DataFrame(data={"page":list(), "line":list(), "word_text":list(), "word_x0":list(),	"word_y0":list(),"word_x1":list(),"word_y1":list(), "index":list()}), type="pandas", headers=["page", "line", "word_text", "word_x0","word_y0","word_x1","word_y1","index"], wrap=False, visible=False)
            ...
            with gr.Column(scale=1):
                ...
                with gr.Tab("Modify existing redactions"):
                    with gr.Accordion("Search suggested redactions", open=True):
                        ...
                        recogniser_entity_dataframe = gr.Dataframe(pd.DataFrame(data={"page":list(), "label":list(), "text":list(), "id":list()}), col_count=(4,"fixed"), type="pandas", label="Click table row to select and go to page", headers=["page", "label", "text", "id"], show_fullscreen_button=True, wrap=True, max_height=400, static_columns=[0,1,2,3])

                        with gr.Row(equal_height=True):                        
                            exclude_selected_btn = gr.Button(value="Exclude all redactions in table")                  
                        
                        with gr.Accordion("Selected redaction row", open=True):
                            selected_entity_dataframe_row = gr.Dataframe(pd.DataFrame(data={"page":list(), "label":list(), "text":list(), "id":list()}), col_count=4, type="pandas", visible=True, headers=["page", "label", "text", "id"], wrap=True)
                ...
                with gr.Tab("Search text to make new redactions"):
                    with gr.Accordion("Search text", open=True):
                        ...
                        all_page_line_level_ocr_results_with_words_df = gr.Dataframe(pd.DataFrame(data={"page":list(), "line":list(), "word_text":list(), "word_x0":list(),	"word_y0":list(),"word_x1":list(),"word_y1":list()}), type="pandas", label="Click table row to select and go to page", headers=["page", "line", "word_text", "word_x0","word_y0","word_x1","word_y1"], show_fullscreen_button=True, wrap=False, max_height=400, show_search="filter")
                        ...
                        with gr.Accordion("Selected row", open=True):
                            selected_entity_dataframe_row_redact = gr.Dataframe(pd.DataFrame(data={"page":list(), "line":list(), "word_text":list(), "word_x0":list(),"word_y0":list(),"word_x1":list(),"word_y1":list()}), type="pandas", headers=["page", "line", "word_text", "word_x0","word_y0","word_x1","word_y1"], wrap=False)
                ...
                with gr.Accordion("Search extracted text", open=True):               
                    ...
                    selected_ocr_dataframe_row = gr.Dataframe(pd.DataFrame(data={"page":list(), "line":list(), "text":list()}), col_count=3, type="pandas", visible=False, headers=["page", "line", "text"], wrap=True)
    ...
    with gr.Tab(label="Identify duplicate pages"):
        ...
        with gr.Accordion("Step 1: Configure and run analysis", open = True):
            ...
            with gr.Accordion("Duplicate matching parameters", open = False):
                with gr.Row():
                    duplicate_threshold_input = gr.Number(value=DEFAULT_DUPLICATE_DETECTION_THRESHOLD, label="Similarity threshold", info="Score (0-1) to consider pages a match.")
                    min_word_count_input = gr.Number(value=DEFAULT_MIN_WORD_COUNT, label="Minimum word count", info="Pages with fewer words than this value are ignored.")
                ...
                min_consecutive_pages_input = gr.Slider(
                    minimum=1, maximum=20, value=DEFAULT_MIN_CONSECUTIVE_PAGES, step=1,
    ...
    with gr.Tab(label="Redaction settings"):       
        ...
        with gr.Accordion("Select entity types to redact", open = True):
                ...
                with gr.Row():
                    max_fuzzy_spelling_mistakes_num = gr.Number(label="Maximum number of spelling mistakes allowed for fuzzy matching (CUSTOM_FUZZY entity).", value=DEFAULT_FUZZY_SPELLING_MISTAKES_NUM, minimum=0, maximum=9, precision=0)
        ...
        with gr.Accordion("Redact only selected pages", open = False):
            with gr.Row():
                page_min = gr.Number(value=DEFAULT_PAGE_MIN, precision=0, minimum=0, maximum=9999, label="Lowest page to redact")
                page_max = gr.Number(value=DEFAULT_PAGE_MAX, precision=0, minimum=0, maximum=9999, label="Highest page to redact")
    ...
    tabular_results_df.select(
        ...
        outputs=[tabular_selected_row_index, tabular_text1_preview, tabular_text2_preview]
    )
...
if __name__ == "__main__":
    if RUN_DIRECT_MODE == "0":
        ...
        if not DIRECT_MODE_INPUT_FILE:
            print("Error: DIRECT_MODE_INPUT_FILE environment variable must be set for direct mode.")
