...
def _safe_existing_path(path: str | Path) -> Path:
    """Validated readable path for workspace CSV/PDF IO."""
...
def _safe_writable_path(path: str | Path) -> Path:
    """Validated writable path (parent created when needed)."""
    p = Path(path).expanduser().resolve()
...
def _fix_dangling_backslash(pattern: str) -> str:
    """Repair patterns like ``David R\\`` that LLMs emit when meaning ``David R\\.``."""
    if pattern.endswith("\\") and not pattern.endswith("\\\\"):
        return pattern + "."
    return pattern


def normalize_regex_patterns(
    patterns: list[str] | str | None,
) -> list[str]:
    """Coerce agent tool args into a list of regex strings.

    LLMs often pass one pipe-separated alternation string instead of a JSON
    array.  Passing that string straight into ``compile_patterns`` would
    iterate characters and break on ``\\`` (e.g. ``Poss\\b``).
    """
    if patterns is None:
        return []
    if isinstance(patterns, str):
        ...
        if not text:
            return []
        return [
            _fix_dangling_backslash(p.strip()) for p in text.split("|") if p.strip()
        ]
    out: list[str] = []
    for item in patterns:
        ...
        if "|" in text:
            ...
            out.append(_fix_dangling_backslash(text))
    return out


def _compile_one_pattern(pattern: str) -> re.Pattern:
    p = _fix_dangling_backslash((pattern or "").strip())
    if not p:
...
def compile_patterns(patterns: list[str] | str | None) -> list[re.Pattern]:
    out: list[re.Pattern] = []
    for p in normalize_regex_patterns(patterns):
        try:
            out.append(_compile_one_pattern(p))
