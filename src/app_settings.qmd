...
* **`EFFICIENT_OCR_MIN_EMBEDDED_IMAGE_PX`**
    * **Description:** When `EFFICIENT_OCR` is enabled and `EFFICIENT_OCR_MIN_IMAGE_COVERAGE_FRACTION` is greater than zero, each embedded image **placement** on the page is considered for image-based OCR routing only if both its width and height are at least this many **PDF points** (PyMuPDF rect units; roughly pixels at 72 dpi). This avoids tiny marks or artifacts that still exceed the area fraction on very small pages. Set to `0` to disable the minimum-size filter (only the area fraction applies). Overridable in the Gradio app (Efficient OCR section), via CLI `--efficient_ocr_min_embedded_image_px`, and in Lambda/direct mode as `efficient_ocr_min_embedded_image_px` in the arguments payload (env: `EFFICIENT_OCR_MIN_EMBEDDED_IMAGE_PX`).
    * **Default Value:** `10`
...
### Review redaction overlay export

These variables apply when exporting the **Review redactions** tab overlay image (JPEG with hollow boxes, optional label abbreviations, and a legend). Output path: `{OUTPUT_FOLDER}/redaction_overlay/{basename}_page{n}_redaction_overlay.jpg`. The FastAPI agent route `POST /agent/export_review_redaction_overlay` uses the same rendering when given `page_image_path` and `boxes`.

* **`REVIEW_OVERLAY_LABEL_ABBREV_CHARS`**
    * **Description:** Number of leading characters of each redaction **label** to draw above each box (centred, with a small gap above the box top edge). Set to `0` to disable label text on the image. Values are clamped to **0–24** after parsing.
    * **Default Value:** `0`

* **`REVIEW_OVERLAY_LABEL_FONT_PX`**
    * **Description:** Font size in pixels for those abbreviation labels. **`0`** means automatic sizing from image width: roughly `max(9, min(16, width // 90))`. Any **positive** value selects a fixed TrueType size, clamped to **6–96** pixels.
    * **Default Value:** `0`

* **`REVIEW_OVERLAY_MAX_PIXELS`**
    * **Description:** If the overlay image’s **width × height** exceeds this value, it is **downscaled** (aspect ratio preserved) before JPEG encoding. Set to **`0`** to disable this cap (no resize for size reasons alone).
    * **Default Value:** `4000000` (4 megapixels)

* **`REVIEW_OVERLAY_MAX_FILE_BYTES`**
    * **Description:** Target **maximum JPEG file size** in bytes. The writer lowers JPEG quality in steps (similar to `SAVE_PAGE_OCR_VISUALISATIONS` page images) until the file is at or under this limit, down to a minimum quality. Values are clamped to a safe range after parsing.
    * **Default Value:** `512000` (500 × 1024 bytes, i.e. 500 KiB)
