...
- [Basic redaction](#basic-redaction)
    ...
    - [Additional AWS Textract / local OCR outputs](#additional-aws-textract--local-ocr-outputs)
    - [Log file outputs and other optional outputs](#log-file-outputs-and-other-optional-outputs)
    - [Downloading output files from previous redaction tasks](#downloading-output-files-from-previous-redaction-tasks)
    - [Basic redaction summary](#basic-redaction-summary)
...
If you are working on a page and you have created boxes manually or modified existing boxes, you are advised to click on the 'Save changes on current page to file' to ensure that they are saved to your output files.
...
The 'View text' table will contain text if you have just redacted a document, or if you have uploaded a '..._ocr_output.csv' file alongside a document file on the Review redactions tab as [described above](#uploading-documents-for-review). If you have just completed a redaction task, you should see this table already filled in.
...
The app can be used to redact Word (.docx), or tabular data files such as xlsx or csv files. For this to work properly, your data file (CSV, Excel) needs to be in a simple table format, with a single table starting from the first cell (A1), and no other information in the sheet. Similarly for .xlsx files, each sheet in the file that you want to redact should be in this simple format.
