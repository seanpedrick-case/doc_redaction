...
![New redaction search menu](https://raw.githubusercontent.com/seanpedrick-case/document_redaction_examples/main/review_redactions/redaction_search_menu.png)
...
![New redaction search with options selected](https://raw.githubusercontent.com/seanpedrick-case/document_redaction_examples/main/review_redactions/redaction_search_friendship_city_options.png)
...
![New redaction search results](https://raw.githubusercontent.com/seanpedrick-case/document_redaction_examples/main/review_redactions/redaction_search_friendship_city_results.png)
...
![New redaction all from specific row](https://raw.githubusercontent.com/seanpedrick-case/document_redaction_examples/main/review_redactions/redact_all_specific_word.png)  
...
![Anonymisation output format](https://raw.githubusercontent.com/seanpedrick-case/document_redaction_examples/main/tabular_files/tabular_anonymisation_formats.PNG)
...
Values are typically small decimals (for example `0.02` for 2% of the page area). Enter **0** to turn off this rule: routing then depends only on the minimum word count (the label in the app notes this as “word count only”). 

**Other limitations.** Even when Efficient OCR is on, pages that stay on the text-only path can still miss text that appears only inside embedded images if no image on that page meets the area fraction above (or the fraction is set to 0). Efficient OCR is also unsuitable when you need signature or face detection on every page (it will be automatically disabled for signature analysis). To disable Efficient OCR entirely, uncheck the 'Efficient OCR' checkbox and run your redaction task.
...
- **SELECTED_LOCAL_TRANSFORMERS_VLM_MODEL** (default: "Dots.OCR"): The VLM model to use. There are a range of options shown in tools/run_vlm.py. Generally, the Qwen 3.5 range of models are the most accurate, and vlm/inference server inference is based on using these models.
...
- **VLM (option 'vlm')**: Recommended for use with the Qwen 3.5 range of models (can set this with the SELECTED_LOCAL_TRANSFORMERS_VLM_MODEL environment variable in config.py). This model is extremely good at identifying difficult to read handwriting and noisy documents. However, it is much slower than the above options.
