...
- **Implemented POST routes** (names align with `api_name` in [app.py](app.py)):  
  ...
  Note: the Gradio Review-tab export helpers are shortened to `page_redaction_review_image` and `page_ocr_review_image` for MCP/tool naming, but the FastAPI routes keep the explicit `export_review_*` names.
